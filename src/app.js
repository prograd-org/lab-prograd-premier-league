...
function createFormation(formation){
  if (formation.length===0){
    ...
    return {forward:formation[0],Midfielder:formation[1],defender:formation[2]}
  }
}
...
namee ="FIFA World Player of the Year";
filterByAward(namee)
function filterByAward(namee){
  let list=[];
  for(var i=0;i<players.length;i++){
   if(players[i].awards[0].name===namee){
       list.push(players[i])
     }
 }
return(list);
...
no_of_time=2;
award_name="African Player of the Year"
...
award_name="FIFA FIFPro World XI";
countrry="Brazil"
function filterByAwardTimes(award_name,countrry){
  let list=[]; 
  for(var i=0;i<players.length;i++){
    if(players[i].country===countrry){
      for(var j=0;j<players[i].awards.length;j++){
         list.push(players[i].awards[j].name)
    }
    }
    }
    return list;
...
agge=30;
teamm="Paris St-Germain";
function filterByNoOfAwardsxTeamxAge(no_of_time,teamm,agge){
  let list=[];
  for(i=0;i<players.length;i++){
    if(teamm===players[i].team && players[i].age<=agge){
        if(no_of_time<=players[i].awards.length){
             list.push(players[i])
        }
      }
    }
